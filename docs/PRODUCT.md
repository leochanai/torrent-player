...
- 设计来源：用户确认的重设计方向 + Harness 产品边界。
- 设计母题：黑匣子信号甲板。
- 设计基调：工业实用风，混入少量复古未来主义仪表感。
- 参考心智：WebTorrent Desktop 的边下边播任务流程、视频质量监控工作台的状态透明度、硬件仪表台的信号轨和诊断读数。
- 不采用方向：盗版资源站首页、影视海报墙、营销落地页、蓝紫科技大屏、白底紫色渐变、通用圆角卡片堆砌。
...
- 视觉记忆点来自 WebTorrent 的真实工作流：来源注入、媒体监视器、peers/Service Worker 状态轨和诊断仪表，而不是内容推荐或装饰图形。
